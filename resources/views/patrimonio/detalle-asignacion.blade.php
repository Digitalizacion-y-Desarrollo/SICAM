...
<header class="flex flex-wrap justify-between gap-4"><h1 class="text-2xl font-bold">Asignación #{{ $asignacion->id }}</h1>@unless($asignacion->fecha_fin)@can('asignaciones.editar')<a data-crud class="action-button" href="{{ route('patrimonio.asignaciones.edit', $asignacion) }}">Reasignar</a>@endcan @endunless</header><section class="form-card"><h2>{{ $asignacion->bien?->nombre }}</h2><p class="mt-2 text-brand">{{ $asignacion->bien?->folio_sicam }}</p><dl class="mt-5 grid gap-5 sm:grid-cols-2">
